...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="bg-black/30 backdrop-blur-xl border border-white/10 rounded-3xl p-6 hover:border-primary/50 transition-all duration-500 hover:scale-105 h-full flex flex-col relative overflow-hidden"
      >
        <!-- Shimmer effect -->
        <div
        ...
        <!-- Avatar -->
        <div
          class="w-24 h-24 bg-gradient-to-br from-primary/20 to-accent/20 rounded-3xl mb-4 flex items-center justify-center mx-auto group-hover:bg-primary/30 transition-all duration-300 group-hover:scale-110 icon-glow-accent relative z-10"
        >
        ...
        <div class="mt-4 text-center relative z-10">
          <button
            (click)="openTeamDetails()"
            class="inline-flex items-center text-primary hover:text-primary-light transition-all duration-300 group-hover:translate-x-1 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-primary/10"
          >
            View Profile
            <i
    ...
    <ng-template #skillTemplate let-skills>
      <span
        *ngFor="let skill of skills.slice(0, 3); trackBy: trackBySkill"
        class="px-3 py-1 bg-primary/20 text-primary text-xs rounded-full border border-primary/30 font-medium"
      >
        {{ skill }}
      </span>
    </ng-template>
  ...
  styles: [
    `
      ...
      .team-card:hover .bg-black\\/30 {
        ...
        box-shadow: 0 25px 50px var(--color-shadow-accent);
      }
      ...
      .team-card:hover .icon-glow-accent {
